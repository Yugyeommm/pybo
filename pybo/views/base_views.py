...
def index(request):
    ...
    # 조회
    # question_list = Question.objects.order_by('-create_date')  # 작성 일시의 역순을 의미
    if kw:
    ...
    # 페이징 처리
    paginator = Paginator(question_list, 15)    # 페이지당 10개씩 보여주기
...
def detail(request, question_id):
    ...
    question = get_object_or_404(
        Question, pk=question_id)  # Question.objects.get(id = question_id)
